...
export class AppComponent {
  ...
  generateName(gender) {
    this.popularNames = this.nameData.sort((a, b) => (a[5] > b[5]) ? 1 : (a[5] === b[5]) ? ((a[4] < b[4]) ? 1 : -1) : -1).filter(x => x[1] === gender.toUpperCase());
    this.popularName = this.popularNames[Math.floor(Math.random() * Math.round(((this.popularNames.length / 100) * 40)))];
  }
}
